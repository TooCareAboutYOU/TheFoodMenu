...
public class HttpListViewData {
	...
	public HttpListViewData(Context context, Handler handler) {
	...
	public void getListViewData(String cid){
		RequestParams params = new RequestParams();
		params.put("cid",cid+"");
		params.put("key",HttpIP.key);
		client=HttpClientContext.getInstance().client;
		client.post(HttpIP.ClassifyListViewURL,params,new JsonHttpResponseHandler("utf-8"){
				...
				try {
					...
					for(int i=0;i<data.length();i++){
						...
						for(int j=0;j<steps.length();j++){
							JSONObject step=data.getJSONObject(j);
